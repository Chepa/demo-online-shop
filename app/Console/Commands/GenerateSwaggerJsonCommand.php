...
class GenerateSwaggerJsonCommand extends Command
{
    protected $signature = 'swagger:generate-json {--output=swagger.json}';
    protected $description = 'Generate Swagger JSON documentation from routes';
    ...
    public function handle(): int
    {
        ...
        file_put_contents($outputPath, json_encode($swagger, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));

        $this->info("Swagger JSON generated successfully: {$outputPath}");
    ...
    {
        $paths = [];
        $apiRoutes = Route::getRoutes()->getRoutesByMethod();
        ...
        foreach ($pathMappings as $route => $operation) {
            [$method, $path] = explode(' ', $route, 2);
            $path = str_replace(['{id}', '{product}', '{order}'], ['{id}'], $path);
